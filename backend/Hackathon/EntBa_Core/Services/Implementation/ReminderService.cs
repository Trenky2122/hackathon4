...
public class ReminderService: BaseService
{
    ...
    public async Task RemindTaxDuties()
    {
        var activeTaxDutiesCloseToEnd = await DbContext.TaxDuties.Include(duty => duty.User).Where(duty =>
            ...
            && (duty.ValidFrom.AddDays(Constants.TAX_TIME_TO_PAY_DAYS).UtcDateTime > DateTimeOffset.UtcNow.AddDays(-1 * Constants.TAX_NOTIFICATION_BEFORE_DAYS)
            || duty.AppealRequested && duty.ValidFrom.AddDays(Constants.TAX_TIME_TO_PAY_DAYS + Constants.TAX_TIME_APPEAL_DAYS).UtcDateTime
            > DateTimeOffset.UtcNow.AddDays(-1 * Constants.TAX_NOTIFICATION_BEFORE_DAYS)))
            .ToListAsync();
